...
class SongsService {
    ...
    async addSong({title, year, genre, performer, albumId}) {
        const id = nanoid(16);
        ...
        const query = {
            ...
            values: [id, title, year, genre, performer, albumId, createdAt, updatedAt],
        };
    ...
    async editSongById(id, {title, year, genre, performer, albumId}) {
        const updatedAt = new Date().toISOString();
        const query = {
            text: 'UPDATE songs SET title = $1, year = $2, genre = $3, performer = $4, albumId = $5, updated_at = $6 WHERE id = $7 RETURNING id',
            values: [title, year, genre, performer, albumId, updatedAt, id],
        };
